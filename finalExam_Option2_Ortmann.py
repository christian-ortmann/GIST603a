...
# 1: Import the necessary libraries for our analysis
#     (a) ensure you are using ArcGIS python interpreter to use arcpy
#     (b) if for some reason some of the libraries are not loading, use "pip install <library name>" in the terminal
# 2: Set up inputs and outputs
# 3: Set up our environment
#     (a) arcpy env
#     (b) set spatial reference (given in intro video)
#     (c) access rasters in geodatabase (there is only one)
#     (d) add the raster we found above and the nest data as raster layer and XY event layer, respectively
#     (e) convert XY event layer to shape file
# 4: Relate distribution of nests to the vegetation types, as per lab manual
#     (a) use the sample tool from spatial analyst to determine how many nests are in each vegetation type, use 'NEAREST' sampling
#     (b) now use the statistics tool from analysis to sum nests by vegetation type, you may need to print field names so you know what field to call
#     (c) conver the summary table to a pandas data frame with the table to numpy array from data access
# 5: Calculate the proportional area of each vegetation type
#     (a) convert the raster layer to a numpy array in the same manner as the previous section
#     (b) use python logic and vectorized calculations to determine proportion of each vegetation type (hint: count of veg type/total count)
# 6: Calculate statistics
#     (a) grab count of each vegetation type from the data frame made in part 4 and add that as a column in our raster data frame from part 5 (hint: may need to loop)
#     (b) calculate chi squared, degrees of freedom, and Cramer's V statistic
#     (c) compare calculated values to chi squared table
# 7: Write your conclusions to the output text file
#------------------------------------------------------------------------------------------
...
import arcpy #to manipulate ArcGIS data
import pandas as pd #to make dataframes
import os #to manipulate file path names
...
#outputs
#--------------------------------------------------
#these are the results and what we will write into at the end
chi2Results
...
#set up 
#--------------------------------------------------
arcpy.env.workspace = veg #add our geodatabase to the workspace
...
try:
    ...
    print('layers successfully created')
except:
    print('issue with layer creation')
...
try:
    ...
    print('sumVegTypes successful')
except:
    print('error in creating sumVegTypes')
...
try: 
    ...
    print('vegLayer attribute table successfully created')
except: 
    print("issue with vegLayer creation")
...
try: 
    ...
    #conditional to set the vars for printing
    ...
    print('issue with calculating statistics')
...
try: 
    ...
    print('there is an issue writing the output to the .txt file')
